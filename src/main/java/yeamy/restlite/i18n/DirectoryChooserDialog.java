...
public class DirectoryChooserDialog extends DialogWrapper {
    ...
    protected JComponent createCenterPanel() {
        ...
        myTree.getSelectionModel().addTreeSelectionListener(this::updatePathFromTree);
    ...
    private void updatePathFromTree(TreeSelectionEvent e) {
        JButton btn = getButton(myOKAction);
        assert btn != null;
        btn.setEnabled(e.getPath().getParentPath() != null);
    }
    ...
    private void createTreeModel() {
        ...
        TreeUtil.sort(myModel, (o1, o2) -> ((DirNode) o1).name.compareToIgnoreCase(((DirNode) o2).name));
    }
...
//        // src/main/ets
        VirtualFile src = dir.findChild("src");
        if (src != null) {
            VirtualFile main = src.findChild("main");
            if (main != null) {
                VirtualFile ets = main.findChild("ets");
                if (ets != null) {
                    DirNode childNode = new DirNode(dir.getName() + "/src/main/ets", ets);
                    addChildDirectory(childNode, ets);
